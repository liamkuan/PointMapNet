...
CAM_NAMES = ['ring_front_right', 'ring_front_left',
...
def create_av2_infos_mp(root_path,
    ...
    metainfo = {
        "dataset_type": f"{split}_dataset",
        "task_name": f"{split}_task"
    }
    infos = dict(data_list=samples, metainfo=metainfo, id2map=id2map)

    info_path = osp.join(dest_path,
                         '{}_map_{}.pkl'.format(info_prefix, split))
    print(f'saving results to {info_path}')
    mmengine.dump(infos, info_path)
