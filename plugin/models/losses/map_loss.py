...
@MODELS.register_module()
class LineAreaLoss(nn.Module):
    def __init__(self, reduction='mean', loss_weight=1.0):
        super(LineAreaLoss, self).__init__()
        self.reduction = reduction
        self.loss_weight = loss_weight
    ...
    def forward(self,
        ...
        reduction = (
            reduction_override if reduction_override else self.reduction)
        temp = pred.unsqueeze(2)
        line1 = torch.cat([temp[:, :-1, :, :], temp[:, 1:, :, :]], dim=2).flatten(0, 1)
        temp = target.unsqueeze(2)
        line2 = torch.cat([temp[:, :-1, :, :], temp[:, 1:, :, :]], dim=2).flatten(0, 1)

        points = torch.cat([line1, line2, line1[:, 0].unsqueeze(1)], dim=1)

        # Calculate the area using the Shoelace formula
        area = 0.5 * torch.abs(torch.sum(points[:, :-1, 0] * points[:, 1:, 1] - points[:, :-1, 1] * points[:, 1:, 0], dim=-1))

        weight = torch.sum(weight.flatten(1), dim=1)
        weight = (weight != 0).float()
        weight = weight.unsqueeze(-1).repeat(1, 19).flatten(0)
        area = torch.sum(area * weight)
        loss_area = self.loss_weight * area
        return loss_area
...
class PointL1Cost(object):
    ...
    def __call__(self, bbox_pred, gt_bboxes):
        num_gts, num_coords = gt_bboxes.shape
        bbox_cost = torch.cdist(bbox_pred, gt_bboxes, p=1)
        return bbox_cost * self.weight
