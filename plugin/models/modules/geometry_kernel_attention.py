...
class GeometrySptialCrossAttention(BaseModule):
    ...
    def forward(self,
        ...
        reference_points_cam = reference_points_cam.float()
...
class GeometryKernelAttention(BaseModule):
    ...
    def __init__(self,
        ...
        offsets = torch.stack(
            torch.meshgrid(x, y, indexing='ij')).permute(1, 2, 0).reshape(grid_h * grid_w, 2)
    ...
    def forward(self,
        ...
        if reference_points.shape[-1] == 2:
            ...
            # (bs, q, 4(z), 4, K^2, 2)
            # sampling_locations = sampling_locations.repeat(1, 1, self.num_heads, 1, 1, 1)
