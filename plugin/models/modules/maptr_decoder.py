...
@MODELS.register_module()
class MapTRDecoderV1(TransformerLayerSequence):
    def __init__(self, *args, return_intermediate=False, **kwargs):
        super(MapTRDecoderV1, self).__init__(*args, **kwargs)
        self.return_intermediate = return_intermediate
...
@MODELS.register_module()
class InstancePointAttention(BaseModule):
    """An attention module used in Deformable-Detr.
    ...
    def forward(self,
        ...
        if reference_points.shape[-1] == 2:
            ...
            sampling_locations = reference_points[:, :, None, :, None, :2] \
                                 + sampling_offsets \
                                 / (offset_normalizer[None, None, None, :, None, :] * reference_points[:, :, None, :,
                                                                                      None, 2:] * 0.5)
        else:
            raise ValueError(
                f'Last dim of reference_points must be'
        ...
        output = output.view(bs, num_iq, num_pt, -1)
...
@MODELS.register_module()
class CustomTransformerLayer(BaseTransformerLayer):
