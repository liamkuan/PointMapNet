...
from plugin.models.modules.spatial_cross_attention import MSDeformableAttention3D
from plugin.models.modules.temporal_self_attention import TemporalSelfAttention
from plugin.models.modules.maptr_decoder import CustomMSDeformableAttention
...
class MapTRPerceptionTransformer(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if modality == "fusion":
            self.fuser = MODELS.build(fuser)  # TODO
    ...
    def forward(
        ...
        **kwargs
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=False):
            mlvl_feats = [item.float() for item in mlvl_feats]
            bev_queries = bev_queries.float()
            bev_pos = bev_pos.float()
...
@MODELS.register_module()
class MapTRPerceptionTransformerV1(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(MapTRPerceptionTransformerV1, self).__init__(**kwargs)
        self.encoder = MODELS.build(encoder)
        self.decoder = MODELS.build(decoder)
        self.embed_dims = embed_dims
        self.num_feature_levels = num_feature_levels
    ...
    def init_layers(self):
        ...
        self.cams_embeds = nn.Parameter(torch.Tensor(self.num_cams, self.embed_dims))
        self.reference_points = nn.Linear(self.embed_dims, 4)  # TODO, this is a hack
        self.can_bus_mlp = nn.Sequential(
    ...
    def attn_bev_encode(
        ...
        **kwargs
        ...
        # add can bus signals
        can_bus = bev_queries.new_tensor(
            [each["ego2global_translation"] + each["ego2global_rotation"] for each in kwargs["img_metas"]]
        )  # [:, :]
        can_bus = self.can_bus_mlp(can_bus)[None, :, :]
        bev_queries = bev_queries + can_bus * self.use_can_bus
    ...
    def forward(
        ...
        **kwargs
        ...
        query = query.unsqueeze(0).expand(bs, -1, -1)
        reference_points = self.reference_points(query_pos)
        reference_points = reference_points.sigmoid().reshape(*reference_points.shape[:-1], 2, 2)
        init_reference_out = reference_points
