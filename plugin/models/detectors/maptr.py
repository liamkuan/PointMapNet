...
class MapTR(Base3DDetector):
    ...
    def __init__(self,
        ...
        super(MapTR, self).__init__(data_preprocessor=data_preprocessor)
    ...
    def loss(self, batch_inputs: dict, batch_data_samples: List[Det3DDataSample]):
        ...
        with autocast(device_type='cuda', dtype=torch.float16, enabled=False):
            ...
            losses = self.head.loss(outs, gt_labels_3d, gt_lines_3d)
        return losses
    ...
    def predict(self, batch_inputs_dict: Dict[str, Optional[Tensor]],
        ...
        for preds in preds_list:
            ...
            instance_data['scores'] = preds['scores']
            instance_data['labels'] = preds['labels']
            instance_data['pts'] = preds['pts']
...
class MapTRv2(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        super(MapTRv2,
              self).__init__(pts_voxel_encoder,
                             ...
                             train_cfg, test_cfg, pretrained, data_preprocessor)
    ...
    def forward_train(self,
        ...
        img_feats = self.extract_feat(img=img, img_metas=img_metas)
        losses = dict()
        gt_bboxes_3d, gt_labels_3d = inputs['gt_bboxes_3d'], inputs['gt_labels_3d']
        gt_bboxes_ignore = inputs.get('gt_bboxes_ignore')
        gt_depth = inputs.get('gt_depth')
        gt_seg_mask = inputs.get('gt_seg_mask')
        gt_pv_seg_mask = inputs.get('gt_pv_seg_mask')
        with autocast(device_type='cuda', dtype=torch.float16, enabled=False):
            img_feats = [item.float() for item in img_feats]
