...
class RCMapNetHead(nn.Module):
    def __init__(
        ...
        super(RCMapNetHead, self).__init__()
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        memory, outs_dec, point_init_ref_points, point_inter_ref_points, init_reference_out, inter_reference_out, image_depths = self.transformer(
        ...
        for lvl in range(outs_dec.shape[0]):
            ...
            if lvl == 0:
                ...
                reference = inter_reference_out[lvl - 1]
            reference = inverse_sigmoid(reference)
            # import pdb;pdb.set_trace()
            # vec_embedding = hs[lvl].reshape(bs, self.num_vec, -1)
            outputs_class = self.cls_branch[lvl](outs_dec[lvl]
                                                   .view(bs, self.num_line, self.num_points, -1)
                                                   .mean(2)).float()
            tmp = self.reg_branch[lvl](outs_dec[lvl]).float()
    ...
    def loss(
        ...
        gt_image_depths = torch.clamp(gt_image_depths, 0, self.pc_range[4] - 1e-5) / self.pc_range[4]
        gt_image_depths = gt_image_depths.flatten(0, 3)
        all_image_depths = all_image_depths.permute(0, 1, 3, 4, 2).flatten(0, 3)
        indices = torch.nonzero(gt_image_depths)
        loss_depth = depth_loss(all_image_depths[indices], gt_image_depths[indices])
        ...
        loss_dict["loss_depth"] = loss_depth * 100
